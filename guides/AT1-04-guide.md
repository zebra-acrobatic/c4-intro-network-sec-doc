...
4.	Start the setup process of the firewall by accepting the terms and conditions:
  ...
  * If the firewall asks you to register at this stage (or any other stage), use the `30 day trial` option. You may have to create or use an existing Sophos ID account on their web site to do this. Keep in mind the Sophos ID used for product registration is not the same as the Sophos ID used in the Sophos Academy, they are not linked and you may need to create a new account for the Sophos ID, [use this link to activate the free trial](https://www.sophos.com/en-us/products/sophos-central/free-trial) you will need to register the firewall (the activation email some times takes a while).
